<template>
  <div class="cart">
      ...
      <div class="cart-footer" v-show="cartList.length">
        ...
        <div class="cart-footer-desc">应付<span>￥{{ costAll }}</span></div>
...
export default {
    computed: {
        ...
        costAll () {
            let a = 0;
            this.cartList.forEach(item => {
                a += this.productDictList[item.id].cost * item.count;
            })
            return a;
        }
    },
    ...
    methods: {
        handleCount (i, count) {
            if (count < 0 && this.cartList[i].count === 1) return;
        ...
        handleDelete (i) {
            this.$store.commit('deleteCart', this.cartList[i].id);
        },
        handleOrder () {
